    ...
    with open(input_file, "r", encoding='utf-8') as reader:
        for qas_id, line in enumerate(reader):
            ...
            if is_training:
                ...
                fake_label = [0] * len(doc_tokens)
                example = LabellingExample(
                    qas_id=qas_id,
                    doc_tokens=doc_tokens,
                    label_seq=fake_label)
                examples.append(example)
    return examples
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for i in range(min(len(all_doc_tokens), max_tokens_for_doc)):
            ...
            if is_training:
                label_seq.append(example.label_seq[orig_index])
            else:
                label_seq.append(0)
        ...
        for input_ids, input_mask, segment_ids, example_indices in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            for i, example_index in enumerate(example_indices):
                ...
                for token_id in range(len(logits)):
                    ...
                    words_scores[orig_id] = max(words_scores[orig_id], logits[token_id][1])
