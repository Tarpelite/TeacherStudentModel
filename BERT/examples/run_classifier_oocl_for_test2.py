...
# for debug
def label_to_idx(label, label_to_ids):
...
def eval(model, args, processor, device, global_step, task_name,label_list,tokenizer,report_path):

    #global global_step
    eval_examples = processor.get_dev_examples(args.data_dir)

    # data debug -statrt
    dev_path = os.path.join(args.data_dir, "dev.csv")
    quotechar = None
    ...
    set_type="dev"
    examples = []
    label_to_ids = {k: v for v, k in enumerate(get_labels())}
    for (i, line) in enumerate(lines):
        ...
        label = label_to_idx(line[1].strip(), label_to_ids)
        print(label)
        examples.append(
...
def split(input_ids_all, input_mask_all, segment_ids_all, label_ids_all, train_size):
    ...
    '''
    input_ids_train = input_ids_all[:train_size]
    input_ids_val = input_ids_all[train_size:]
    input_mask_train = input_mask_all[:train_size]
    input_mask_val = input_mask_all[train_size:]
    segment_ids_train = segment_ids_all[:train_size]
    segment_ids_val = segment_ids_all[train_size:]
    label_ids_train = label_ids_all[:train_size]
    label_ids_val = label_ids_all[train_size:]
